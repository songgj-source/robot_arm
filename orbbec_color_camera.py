...
# 실내 형광등/LED 조명(60Hz 교류)과 컬러 센서의 노출 타이밍이 안 맞으면 화면에
# 밝기가 주기적으로 출렁이는 플리커가 생긴다. XN_STREAM_PROPERTY_FLICKER는
# PrimeSense 계열 칩(Astra S 포함)이 지원하는 레거시 안티플리커 속성으로,
# 0=끄기, 1=50Hz, 2=60Hz. 한국 콘센트 기준 60Hz로 고정.
_FLICKER_HZ_60 = 2


def _enable_anti_flicker(color_stream):
    try:
        color_stream.set_property(_openni2.XN_STREAM_PROPERTY_FLICKER, ctypes.c_uint64(_FLICKER_HZ_60))
    except Exception as e:
        print(f"[Orbbec] 경고: 안티플리커(60Hz) 설정을 켤 수 없습니다: {e}")

# 자동노출(AE) 상태에서는 위 FLICKER 힌트를 드라이버가 사실상 무시해서 플리커가
# 거의 안 줄어드는 것을 실측으로 확인했다. AE를 끄고 노출을 60Hz 교류 주기(16.7ms)의
# 배수로 수동 고정하면(실측 33ms ≈ 1/30초 = 2/60Hz에서 밝기 변동폭이 절반 이하로
# 줄어듦, p2p 6.6% -> 3.25%) 훨씬 확실히 줄어든다. 대신 조명이 바뀌면(창밖 햇빛,
# 다른 조명 on/off) 화면이 자동으로 밝기를 재조정하지 못해 너무 밝거나 어둡게 나올
# 수 있다는 트레이드오프가 있다 — 사용자가 이 트레이드오프를 감수하기로 확인함.
_MANUAL_EXPOSURE_MS = 33


def _lock_manual_exposure(color_stream):
    try:
        color_stream.set_property(_openni2.ONI_STREAM_PROPERTY_AUTO_EXPOSURE, ctypes.c_int(0))
        color_stream.set_property(_openni2.ONI_STREAM_PROPERTY_EXPOSURE, ctypes.c_int(_MANUAL_EXPOSURE_MS))
    except Exception as e:
        print(f"[Orbbec] 경고: 수동 노출 고정을 켤 수 없습니다: {e}")

_oni_lock = threading.Lock()
_oni_initialized = False

# Astra S 컬러 센서 하드웨어 결함: 왼쪽 가장자리 ~20px 구간에 비네팅 + 죽은 컬럼이
# 섞여 있어 밝은 배경에서도 어둡게 찍힌다(실측: col 12~14 부근에서 밝기가 200대에서
# 10~70까지 급락). 옆 컬럼 값으로 평평하게 채우면 그 경계에서 오히려 새 seam(줄)이
# 생기므로, 해당 구간을 통째로 잘라내고 원래 너비로 다시 늘려서 결함 자체를 지운다.
_DEFECT_CROP_COLS = 20


def _fix_left_edge_defect(img_bgr):
    h, w = img_bgr.shape[:2]
    cropped = img_bgr[:, _DEFECT_CROP_COLS:]
    ...
    def _run(self):
        try:
            ...
            try:
                ...
                pass
            self.stream.start()
            _enable_anti_flicker(self.stream)
            _lock_manual_exposure(self.stream)
        except Exception as e:
            self._open_error = e
        ...
        while self._running:
            ...
            img_bgr = _fix_left_edge_defect(img_bgr)
...
class ThreadedOrbbecRGBDCamera:
    ...
    def _run(self):
        try:
            ...
            try:
                self.depth_stream.configure_mode(self.width, self.height, self.fps, openni2.PIXEL_FORMAT_DEPTH_1_MM)
            except Exception:
                pass
            ...
            _enable_anti_flicker(self.color_stream)
            _lock_manual_exposure(self.color_stream)
        except Exception as e:
            self._open_error = e
            self._opened_event.set()
            return
        ...
        while self._running:
            ...
            color_bgr = _fix_left_edge_defect(color_bgr)
